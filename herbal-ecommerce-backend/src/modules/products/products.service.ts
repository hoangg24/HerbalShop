...
export const getProducts = async (query: GetProductsQuery) => {
  ...
  const limit = parseInt(query.limit) || 12; // ✅ default limit = 12
  const sortBy = query.sortBy || "createdAt"; // ✅ default sortBy
  ...
  const [products, total] = await Promise.all([
    prisma.product.findMany({
      ...
      skip,
      take: limit,
      orderBy: { [sortBy]: sortOrder }, // ✅ dùng biến đã có default
...
export const getMyProducts = async (
  ...
  if (vendor.status !== "active")
    throw { status: 403, message: "Shop của bạn chưa được duyệt" };
...
export const updateProduct = async (
  ...
  if (
    input.salePrice &&
    input.salePrice >= (input.price ?? product.price.toNumber())
  ) {
    throw { status: 400, message: "Giá khuyến mãi phải nhỏ hơn giá gốc" };
  }

  const slug = input.name
    ? await generateSlug(input.name, productId)
    : undefined;
...
export const deleteProduct = async (userId: string, productId: string) => {
  ...
  if (orderCount > 0) {
    throw {
      status: 400,
      message:
        "Không thể xóa sản phẩm đã có trong đơn hàng, hãy ẩn đi thay thế",
    };
  }
...
export const deleteProductImage = async (userId: string, imageId: string) => {
  ...
  if (image.isPrimary)
    throw {
      status: 400,
      message: "Không thể xóa ảnh chính, hãy đặt ảnh khác làm chính trước",
    };
